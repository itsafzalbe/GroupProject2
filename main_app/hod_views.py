...
def admin_home(request):
    ...
    context = {
        'page_title': "Adminstrative Dashboard",
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            try:
                ...
                return redirect(reverse('add_stafff'))
...
def edit_course(request, course_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            messages.error(request, "Could not Update")
    return render(request, 'hod_templat/edit_course_template.html', context)
    ...
    if request.method=='POST':
        if form.is_valid():
            try:
                ...
                messages.error(request, "Session Coould not be updated "+ str(e))
...
def delete_subject(request, subject_id):
    subject = get_object_or_404(Session, id = subject_id)
    subject.delete()
...
def delete_session(request, session_id):
    ...
    try:
        ...
        messages.error(request, "There are other students assigned to this session. Please move them to another session")
...
def get_admin_attendance(request):
    ...
    attendance_data_id = request.POST.get('attendance_date_id')
    try:
        subject = get_object_or_404(Subject, id = subject_id)
        session = get_object_or_404(Session, id = subject_id)
        attendance = get_object_or_404(Attendance, id = attendance_data_id, session = session)
        attendance_reports = AttendanceReport.objects.filter(attendance=attendance)
        json_data = []
        for report in attendance_reports:
            data = {
                "session": str(report.status),
...
def admin_view_profile(request):
    ...
    context = {'form': form, 'page_title': "View/Edit Profie"}
    if request.method == 'POST':
        try:
            if form.is_valid():
                first_name = form.cleaned_data.get('first_name')
...
#     context = {
#         'pager_title':'Send Notifications To Staff',
#         'allStaff': staff
#     }
...
#     context = {
#         'page_title': "Student Notificatons To Students",
#         'students': student
#     }
